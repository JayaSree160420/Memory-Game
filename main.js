...
let moves;
let second=0,minute=0,hour=0,interval,totalGameTime,starRating;
let openedCards=[];
let matchedCards=[];
 ...
 {
     //shuffle cards
     let shuffleImages=shuffle(imgElementsArray);
     for(i=0;i<shuffleImages.length;i++)
     {

        //remove all images from previous games from each card 
         cardElements[i].innerHTML="";

         //add the shuffled images to each card
         cardElements[i].appendChild(shuffleImages[i]);
         cardElements[i].type=`${shuffleImages[i].alt}`;
